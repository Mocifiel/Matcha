...
class AttentiveStatsPool(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.global_context_att:
            ...
            context_std = torch.sqrt(torch.var(x, dim=-1, keepdim=True) + 1e-6).expand_as(x)
        ...
        std = torch.sqrt(residuals.clamp(min=1e-6))
...
class ECAPA_TDNN(nn.Module):
    def __init__(self, feat_dim=80, channels=512, emb_dim=192, global_context_att=False,
        ...
        if feat_type == 'fbank':
            ...
            #self.feature_extract = torch.hub.load(r'D:\gitrepo\SpeakerVerification\UniSpeech-main\downstreams\speaker_verification\models\s3prl\upstream\wavlm', feat_type, source='local')
            #else:
    ...
    def get_feat(self, x):
        ...
        if self.feat_type != "fbank" and self.feat_type != "mfcc":
            ...
            if isinstance(x, (list, tuple)):
                ...
                x = x.unsqueeze(0)
            # print(f'shape after feature extract={x.shape}')
